...
import { isGradeScale, type GradeTarget, type GradeTargetInput } from "../app/grade-targets";
...
export async function saveGradeTarget(workspaceId: string, input: GradeTargetInput) {
  ...
  const rows = await sql<Record<string, unknown>[]>`
    INSERT INTO grade_targets (
      workspace_id, subject_id, grade_scale, completed_stage, completed_grade,
      ...
      ${workspaceId}, ${input.subjectId}, ${input.gradeScale}, ${input.completedStage},
      ${input.completedGrade}, ${input.completedMark}, ${input.completedMax},
      ${input.completedPercent}, ${input.completedWeight}, ${input.remainingStage},
      ${input.targetGrade}, ${input.paperTargetPercent}, ${now}, ${now}
    )
